...
describe('todoListApi', () => {
  it('getTodoById - fulfilled', async () => {
    fetchMock.mockOnceIf(`https://jsonplaceholder.typicode.com/todos/${mockDataFromApi.id}`, () =>
      Promise.resolve({
        status: 200,
    ...
    act(() => {
      void triggerGetTodoById(mockDataFromApi.id)
    })
    ...
    expect(result.current[1]).toMatchObject({
      status: 'fulfilled',
      data: {
        id: mockDataFromApi.id.toString(),
